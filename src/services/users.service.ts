...
class UserService {
  ...
  public async search(keyword): Promise<User[]> {
    const users = await this.users.find({ fullName: { $regex: keyword, $options: 'i' } }).select('fullName profilePicture slug');
    return users;
  }
  ...
  public async findUserBySlugOrId(userId: string): Promise<User> {
    ...
    if (isValidObjectId(userId)) {
      user = await this.users
        .findOne({
          $or: [{ _id: userId }, { slug: userId.toLowerCase() }],
        })
        .select('-__v -password');
    } else {
  ...
  public async follow(currentUser: User, otherUser_id: string, follow: boolean): Promise<User> {
    ...
    if (follow) {
      otherUser = await this.users
        .findByIdAndUpdate(otherUser_id, { $addToSet: { followers: currentUser } }, { new: true })
        .select('-__v -saltSecret -password');
      currentUser = await this.users
        .findByIdAndUpdate(currentUser, { $addToSet: { following: otherUser_id } }, { new: true })
        .select('-__v -saltSecret -password');
      ...
      otherUser = await this.users
        .findByIdAndUpdate(otherUser_id, { $pull: { followers: currentUser } }, { new: true })
        .select('-__v -saltSecret -password');
      currentUser = await this.users
        .findByIdAndUpdate(currentUser, { $pull: { following: otherUser } }, { new: true })
        .select('-__v -saltSecret -password');
    }
    return otherUser;
  }
  ...
  public async findNotifications(user: User): Promise<Notification[]> {
    const notifications = notificationModel.find({ receiver: user }).populate('sender', '_id fullName profilePicture slug').sort('-createdAt');
